...
class GetDemand:
    ...
    def __init__(self):
        ...
        self.cookies = requests.cookies.RequestsCookieJar() # 实例化一个session,建立一个session会话

    def demand(self, method, url, data=None):
        ...
        if method == 'GET':
            resp = requests.request(method, url=url, params=data,cookies=self.cookies)# 调用get方法，使用params传参（url传参）
            ...
            resp = requests.request(method, url=url, data=data,cookies=self.cookies)# 调用post方法，使用data传参（表单传参）
            logger.info('response: {0}'.format(resp.text))
            self.cookies.update(resp.cookies)
            return resp
    ...
    # def close(self):
    #     self.cookies.close()#关闭session

if __name__ == '__main__':
    request = GetDemand()
    data = {"mobilephone": "13670287832", "pwd": "123456"}
    request.demand('get',"http://test.lemonban.com/futureloan/mvc/api/member/login",data=data)