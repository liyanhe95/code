...
class DoExcel:
    ...
    def get_data(self,sheet_name):
        ...
        for i in range(2,sheet.max_row+1):
            ...
            if type(cases.expected) == int:
                cases.expected = str(cases.expected)
            case.append(cases)  #将case放入cases列表里
        return case
...
if __name__ == '__main__':
    ...
    for item in rest :
        ...
        if resp.text == item.expected:
            do_excel.write_result('invest',item.id+1,resp.text,'PASS') #item.id+1是当前的row，resp.text真实值
        else:
            do_excel.write_result('invest',item.id+1,resp.text,'FAIL')
